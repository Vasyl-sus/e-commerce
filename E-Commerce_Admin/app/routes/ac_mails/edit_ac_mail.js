...
class EditACMail extends Component {
	constructor(props) {
		super(props)

		this.state = {languages:[], preview: null, image: [], image_link:''}

  }

  componentDidMount() {
    this.changeLanguage(this.props.initialValues.toJS().country);
  ...
  editACMail(obj){
    ...
    this.props.editACMail(obj.id, acmail_data);
    console.log(obj);
  }
  ...
  renderSelect1 = ({ input, placeholder, type, label, options, meta: { touched, error, warning, valid } }) => (
    ...
    const { preview, image_link } = this.state;
    ...
    const img_style = {
      ...
      onRequestClose={this.closeModal.bind(this)}>
        ...
        <header className={`confirm_box clearfix`}>
          <h2 className="center">Uredi maila</h2>
        </header>
      	<form onSubmit={handleSubmit(this.editACMail.bind(this))} >
          <div className={`modal-body row`}>
            ...
            <div className="col-lg-6">
              <Field name="country" placeholder="Izberi državo..." options={this.props.countries.map(c => {return {label: c.name, value: c.name}})} self={this} component={this.renderSelect1} label="Država"/>
            </div>
            <div className="col-lg-6">
            ...
            <div className="col-lg-6">
              ...
              { preview == null && image_link ? <img style={img_style} src={image_link} /> : "" }
            </div>
          </div>
