...
import {addNewExpense, editExpense} from '../../actions/expense_configurator_actions';
...
class EditExpenseForm extends Component {
  ...
  addExpense(data) {
    var sendData = {};
    data = data.toJS();
    // data.country = this.props.formValues.country;
    data.category = this.props.formValues.category;
    ...
    this.props.editExpense(sendData, this.props.formValues.id);
    console.log(sendData)
  }

  componentWillReceiveProps(nextProps){
    if(this.props.formValues && this.state.additional[0] && this.state.additional[0].content==null){
      this.setState({additional:this.props.formValues.additional_fields});
    }
  }
    ...
    return this.state.additional.map((el, i) =>
        <div key={i}>
         ...
         <Field
          data={el.content}
          name={'add'+i}
          component={renderAdditionalField}
          type='text'
    ...
    return(
      <div key={index}>
         <Field
          ...
          onChange={this.handleChange.bind(this, i)}
          />
          <button className="btn btn-set"><i className="fa fa-plus" aria-hidden="true"></i></button>
      </div>
    )
  }
  ...
  render(){
    ...
    return (
      <form onSubmit={handleSubmit(this.addExpense.bind(this))} >
        <span className="span-conf">Konfigurator stroška - nastavljanje stroška</span>
        <h2 className="conf-title">{formValues && formValues.name}</h2>
        ...
        <div>
          ...
          <div className="row sp-bet">
            <div className="col-lg-6 col-md-9 sp-bet">
              ...
              <div className="flex">
                <label>Izdelek:</label>
                <Field name="products" placeholder="Izberi izdelek..." options={this.state.products.map(p => {return {label: p.name, value: p.id}})} component={renderSelectMulti}/>
              </div>
              :
               formValues.billing_type && (formValues.billing_type == 'gift') ?
              <div className="flex">
                <label>Darilo:</label>
                <Field name="gifts" placeholder="Izberi darilo..." options={this.state.gifts.map(g => {return {label: g.name, value: g.id}})} component={renderSelectMulti}/>
              </div>
              :
               formValues.billing_type && (formValues.billing_type == 'accessory') ?
              <div className="flex">
                <label>Dodatek:</label>
                <Field name="accessories" placeholder="Izberi dodatek..." options={this.state.products.map(a => {return {label: a.name, value: a.id}})} component={renderSelectMulti}/>
              </div>
              :
               formValues.billing_type && (formValues.billing_type == 'deliverymethod') ?
              <div className="flex">
                <label>Način dostave:</label>
                <Field name="deliverymethods" placeholder="Izberi način dostave..." options={this.state.deliverymethods.map(d => {return {label: d.code, value: d.id}})} component={renderSelectMulti}/>
              </div>
              : ""}
            </div>
          </div>
  ...
  const renderField = ({ input, field, label, value, inputclass, place, type, meta: { touched, error, warning } }) => (
    <input type={type} placeholder={place} value={value} {...input} className={`${touched && (error || warning) ? 'input_error' : ''} form-control configurator-input`} />
  )

  const renderAdditionalField = ({ input, field, data, label, value, inputclass, place, type, meta: { touched, error, warning } }) => (
  ...
  const renderSelectMulti = ({ input, disabled, placeholder, type, label, options, meta: { touched, error, warning } }) => {
...
function mapStateToProps(state) {
  ...
  return {
    formValues:nextState.form.EditExpenseForm && nextState.form.EditExpenseForm.values,
