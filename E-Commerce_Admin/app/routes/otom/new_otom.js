...
class NewOTOM extends Component {
	constructor(props) {
		super(props)

		this.state = {languages:[], selectedTherapies:[], preview: null, image: [], t: []}
	}

  componentDidMount() {

    var t = this.props.therapies.map(t => {
  ...
  createOTOMail(obj)
  {
    ...
    if(obj.therapies.length > 0) {
      if(forward) {
        ...
        obj.post_image = this.state.image

        this.setState({preview: null, image:[]})
        this.props.CreateOTOMail(data);
        this.props.reset();
      }
    } else {
  ...
  renderTherQ(row, index){
    return(
      <div key={index}>
        ...
        <input className={`form-control`} type="number" name={row.index} onChange={this.changedQ.bind(this, row.value)}/>
      </div>
    )
  }
      ...
      <div>
        ...
        style={otherModalStyles}>
          ...
          <header className={`confirm_box clearfix`}>
            <h2 className="center">Dodaj OTO mail</h2>
          </header>
        	<form onSubmit={handleSubmit(this.createOTOMail.bind(this))}>
