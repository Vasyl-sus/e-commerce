...
class ReplicateTestimonial extends Component {
	constructor(props) {
		super(props)
  ...
  componentDidMount() {
    ...
    this.setState({show_image: this.props.a_profile_image})
  ...
  replicateTestimonial(obj)
  {
    ...
    obj.content = this.replaceAll(obj.content, mapObj)
    ...
    testimonial_data.append("sort_order", 1);
    ...
    var images = [];
    var instagram_img = [];
    if(this.state.show_image && this.state.show_image.length != 0)
    {
      profile_img.push(this.state.show_image);
    }
    if(this.state.show_images && this.state.show_images.length != 0)
    {
      images.push(this.state.show_images);
    }
    if(this.state.show_instagram_images && this.state.show_instagram_images.length != 0)
    {
      for(var i=0; i<this.state.show_instagram_images.length; i++)
      {
        instagram_img.push(this.state.show_instagram_images[i]);
      }
    }

    var existing_images = {};
    existing_images.profile_image = profile_img;
    existing_images.timeline_image = images;
    existing_images.instagram_images = instagram_img;
    obj.existing_images = JSON.stringify(existing_images);

    testimonial_data.append("existing_images", JSON.stringify(existing_images));
    this.props.ReplicateTestimonial(testimonial_data);
    console.log(obj);
  }
    ...
    const { preview, images_preview, instagram_images,show_instagram_images } = this.state;
      ...
      style={otherModalStyles}>
        ...
        <header className={`confirm_box clearfix`}>
          <h2 className="center">Dupliciraj izjavo</h2>
        </header>
        <form onSubmit={handleSubmit(this.replicateTestimonial.bind(this))}>
...
const validate = values => {
  ...
  if (!values.favourite) {
    errors.favourite = 'Obvezno polje';
  }
...
export default compose(
  reduxForm({
    form: 'ReplicateTestimonialForm',
    enableReinitialize: true,
    validate
  }), connect(null, mapDispatchToProps)
)(ReplicateTestimonial);
