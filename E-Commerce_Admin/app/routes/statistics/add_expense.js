...
class ExpenseData extends Component {
	constructor(props) {
		...
		this.state = {startDateFrom:new Date(), activeCountry: null, expenseID: null, expenseData: null,
    additional_fields: null, field_key:null, dateValue: 0}
  ...
  getExpenseID(expense) {
    this.setState({expenseID: expense.id, expenseData: {name: expense.name, expense_type: expense.expense_type, id: expense.id,
    value: expense.value, billing_period: expense.billing_period, billing_type: expense.billing_type}, additional_fields: expense.additional_fields })
  }
  ...
  renderExpenseSubSections(sub_section, index) {
    ...
    const c = localCountries.map(c => { return {label: c, value: c}});
      ...
      style={otherModalStyles}>
        <header>
          <h2 className="p-3">Vnos stroška</h2>
        </header>
        <div className="padding-modal-stat px-3 row">
           <div className="col-md-12 pt-4">
             <h3>1. Izberi strošek</h3>
             <div className="new-cost-modal">
               <div className="flex">
           ...
           {expenseID &&
             <div className="px-3">
               <h3 className="m-t-30">2. Vnesi podatke</h3>
