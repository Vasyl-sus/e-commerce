...
import {getDailyStatistics, changedUTMStatistics, changedUTMStatistics1, addExpense, changeExpenseCountry, getUTMStats} from '../../actions/statistics_actions';
...
class MonthlyStatistics extends Component {
    ...
    componentDidMount()
    {
      ...
      this.props.getDailyStatistics('month');
    };
    ...
    checkUTM(type, data) {
      console.log("checkUTM called with:", type, data);
      if (!this.props.data_report.utm_filters) {
        console.error("utm_filters is undefined");
        return;
      }
      console.log("UTM filters:", this.props.data_report.utm_filters);
      if (data.active == 1) {
        console.log("Calling changedUTMStatistics");
        this.props.changedUTMStatistics(data, type, this.props.data_report.utm_filters, 'month');
      } else {
        console.log("Calling changedUTMStatistics1");
        ...
        <div className="content-wrapper container-fluid">
          <div className="row">
            <div className="col-md-12">
              <h2 className="box-title">Mesečni pregled</h2>
            </div>
          </div>
          <div className="box box-default pb-5 mb-4">
            ...
            <div className="row">
              <div className="col-md-2 p-t-20">
                <div className="box-body from-top">
                  {stats && <Bar data={stats} width={200} height={450} />}
                </div>
              </div>
            ...
            { super_admin ?  <div className="col-md-6 p-t-30">
                <div className="row">
                  <div className="col-md-12 expenses-block">
                    <span className="stat-span-inf daily-stats-inf">ODHODKI</span>
                    <span className="stat-big-num">{data_report.total_expenses} €</span>
                    <button className="ml-4 edit-customer-btn btn-cust-small expenses-cta" onClick={this.openNewModal.bind(this)}><i className="fa fa-plus" aria-hidden="true"></i>DODAJ STROŠEK</button>
                    <div className="row">
                      {data_report.expenses &&
                        data_report.expenses.map(this.renderNewSub.bind(this))
                      }
                    </div>
                  </div>
                </div>
          ...
          { super_admin ? <div className="box box-default">
            <div className="row">
              <div className="col-md-3 pt-3">
                <span className="date-stat">Dnevne stroške</span><br/><br/>
              </div>
              <div className="d-block col-md-3">
                ...
                <Select
                  className="form-white"
                  name=""
                  placeholder="Izberi država..."
                  value={this.state.expenseSelectedCountry}
                  options={c}
                  multi={false}
                  onChange={this.changeExpenseCountriesSelect.bind(this)}
                />
              </div>
              <div className="col-md-12">
                <div className="table-responsive cost-table p-4">
                  {this.renderExpensesInfo(data_report.langExpenses)}
                </div>
              </div>
            </div>
          </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    getDailyStatistics: (type) => {
      dispatch(getDailyStatistics(type))
    },
