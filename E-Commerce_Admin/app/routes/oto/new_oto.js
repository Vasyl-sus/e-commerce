...
class NewOto extends Component {
	constructor(props) {
		super(props)
  ...
  CreateOto(obj)
  {
    ...
    if(obj.payment_method_id && obj.payment_method_id.length > 0) {
      obj.payment_method_id = obj.payment_method_id.map(a => {return a.value});
    } else {
  ...
  renderSelect1 = ({ input, placeholder, type, label, options, meta: { touched, error, warning, valid, multi } }) => (
    ...
    const { handleSubmit } = this.props;
      ...
      style={otherModalStyles}>
        ...
        <header className={`confirm_box clearfix`}>
          <h2 className="center">Dodaj OTO</h2>
        </header>
      	<form onSubmit={handleSubmit(this.CreateOto.bind(this))}>
