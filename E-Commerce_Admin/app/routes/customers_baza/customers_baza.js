...
import {getCustomersBaza} from '../../actions/customers_bd_actions';
...
class CustomersBazaProfiles extends Component {
    constructor(props) {
      super(props);

      this.state = {inputDate: Moment(new Date()), newModal: false, editModal: false, activeRow:null, enabled:false, activeStar:0, activeFilter:false,
      searchString:'', pageNumber:1, pageLimit:15, count:0, customerID:null, addBadgeBtn:false, selectedCountries:[], selectedCriteria:this.props.criteria[0]};
    }
    ...
    componentDidMount(){
      ...
      this.props.getCustomersBaza(this.state.selectedCriteria.value);
    };
    ...
    changeCriteria(event) {
      ...
      if(criteria == '1' || criteria == '2' || criteria == '3' || criteria == '4') {
        ...
        browserHistory.replace(location);
        this.resetPageNumber(location);
        this.props.getCustomersBaza(criteria);
      }
    }
         ...
         <div className="content-wrapper container-fluid">
            <div className="row">
              <div className="col-md-12">
                <h2 className="box-title">Stranke - Baza</h2>
              </div>
            </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    getCustomersBaza: (criteria) => {
      dispatch(getCustomersBaza(criteria));
    },
...
function mapStateToProps(state) {
  ...
  return {
    customers: nextState.customers_bd_data.customers_baza,
    customer_details: nextState.customer_profile_data.customer,
    customersCount: nextState.customers_bd_data.customersCount_baza,
    ...
    criteria: [{label: '1. kriterij - 6 MES. BREZ NAROČILA (1x v 6-18 MES.)', value: '1'},
              {label: '2. kriterij - 6 MES. BREZ NAROČILA (2x ali več v 6-24 MES.)', value: '2'},
              {label: '3. kriterij - 3 MES. BREZ NAROČILA (1x ali več v 3-36 MES.)', value: '3'},
              {label: '4. kriterij - 6 MES. BREZ NAROČILA (2x ali več v 6-36 MES.)', value: '4'}]
  }
}
