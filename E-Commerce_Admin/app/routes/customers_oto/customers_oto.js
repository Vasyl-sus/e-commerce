...
import {showCustomerData, editProfile, setInitialValuesProfile, deleteProfile, setInitialValuesNewOrder, getCustomerDetails} from '../../actions/customer_profile_actions';
import {getCustomersOTO} from '../../actions/customers_bd_actions';
...
class CustomersOtoProfiles extends Component {
    constructor(props) {
      super(props);

      this.state = {newModal: false, editModal: false, searchString:'', pageNumber:1,
                    pageLimit:15, customerID:null, addBadgeBtn:false, selectedCountries:[]};
    }
      ...
      return (
        <tbody style={{backgroundColor: color}} key={index}>
          <tr className="pointer">
            ...
            <td className="center" onClick={this.showCustomerProfileData.bind(this, customer)}>{customer.otom_sent_count}</td>
            <td className="center" onClick={this.showCustomerProfileData.bind(this, customer)}>{customer.otom_opened_count}</td>
            <td className="center" onClick={this.showCustomerProfileData.bind(this, customer)}>{customer.otom_used_count}</td>
            ...
            <td>
              <button type="button" className="ml-3 call-pink-btn" onClick={this.call.bind(this, customer, true)}>KLIČI</button>
            </td>
          </tr>
          <tr>
            <td className={`hidden-tr ${this.state.customerID == customer.id ? '' : 'closed'}`} colSpan="12">
              <div className="main-hidden container-fluid">
                <div className="d-flex align-items-center row customer-name">
                  ...
                  {customer.upsale_count>0 && <span className="customer-flags gold-badge">{customer.upsale_count}x UPSELL</span>}
                  {customer.no_upsale_count>0 && <span className="customer-flags red">x BREZ UPSELL</span>}
                  {customer.declined_count>0 && <span className="customer-flags red">{customer.declined_count}x NEPREVZEM</span>}
                  {customer.badges &&
                    customer.badges.map((u, index) => {
                      return (
        ...
        <div className="content-wrapper container-fluid">
          <div className="row">
            <div className="col-md-12">
              <h2 className="box-title">Stranke - OTO</h2>
            </div>
          </div>
              ...
              <div className="col-md-12">
                <div className="table-responsive no-padding">
                  <table className="table">
                    <thead>
                      <tr>
                        <th className="left table-pad-l-8"></th>
                        ...
                        <th className="center table-pad-l-8">Poslane OTO</th>
                        <th className="center table-pad-l-8">Odprte OTO</th>
                        <th className="center table-pad-l-8">Uporabljene OTO</th>
                        <th className="left table-pad-l-8"></th>
                        <th className="left table-pad-l-8"></th>
                      </tr>
                    </thead>
                    {customers.map(this.renderSingleCustomer.bind(this))}
                  </table>
                </div>
...
function mapStateToProps(state) {
  ...
  return {
    customers: nextState.customers_bd_data.customers_bd,
    customer_details: nextState.customer_profile_data.customer,
    customersCount: nextState.customers_bd_data.customersCount_bd,
...
export default compose(
  reduxForm({
    form: 'CustomerOtoProfileForm',
    enableReinitialize: true
  }), connect(mapStateToProps, mapDispatchToProps)
)(CustomersOtoProfiles);
