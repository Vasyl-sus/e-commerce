...
class EditDiscount extends Component {
  ...
  componentDidMount()
  {
    this.setState({dateFrom: new Date(this.props.initialValues.toJS().date_start), dateTo: new Date(this.props.initialValues.toJS().date_end)})
  };
    ...
    obj.accessories = [];
    this.props.EditExistingDiscount(obj);
    console.log(obj);
	}
  ...
  handleDateChange(dates) {
    ...
    var type = formValues && formValues.type;

    const countries = this.props.formValues && this.props.formValues.countries.map(country => {
      return country.value
    })

    const freeTherapiesCountry = this.props.free_therapies.filter(t => {
      if (countries && countries.length > 0 && countries.indexOf(t.country) >= 0) {
        return t
      }
    })
    const freeAccessoriesCountry = this.props.free_accessories.filter(t => {
      if (countries && countries.length > 0 && countries.indexOf(t.country) >= 0) {
        return t
      }
    })
    return (
      ...
      style={otherModalStyles}>
        ...
        <header className={`confirm_box clearfix`}>
          <h2 className="center">Uredi kupon</h2>
        </header>
      	<form onSubmit={handleSubmit(this.editDiscount.bind(this))} >
          <div className={`modal-body row`}>
            ...
            {type == "Shipping" ? "" : <Field name="discount_value" place="Vnesi vrednost" inputclass="col-lg-12" type="number" component={renderField} label="Vrednost"/> }
            {type == "Shipping" ? "" : <Field name="min_order_amount" place="Vnesi minimalno vrednost naročila" inputclass="col-lg-12" type="number" component={renderField} label="Minimalna vrednost naročila"/> }
            <div className={`col-lg-12`}>
            ...
            {type == "Individual" ? <div className="col-lg-12">
            ...
            {type == "Free product" ? <div className="col-lg-12">
              <Field
                name="free_therapies"
                placeholder="Izberi terapije..."
                inputclass="col-lg-6"
                options={freeTherapiesCountry.map(t => {return {label: t.name, value: t.id}})}
              ...
              <Field
                ...
                inputclass="col-lg-6"
                options={freeAccessoriesCountry.map(t => {return {label: t.name, value: t.id}})}
                component={renderSelect}
                multi={true}
                label="Free Dodatki"
              />
...
function mapStateToProps(state) {
  ...
  return {
    formValues: nextState.form.EditDiscountForm && nextState.form.EditDiscountForm.values
  }
}
