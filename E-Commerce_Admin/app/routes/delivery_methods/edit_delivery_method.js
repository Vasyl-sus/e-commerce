...
class EditDeliveryMethod extends Component {
	constructor(props) {
		...
		this.state = {
      additional:[], translations: null, checked: false, therapies:[], preview: null, image: [], post_image: null
    }
	}
  ...
  componentDidMount() {
    ...
    if(translations) {
      ...
      lang = Object.keys(this.props.langs);

      let additional = [...this.state.additional];
      for(var i=0; i<lang.length; i++) {
        if(keys.includes(lang[i]))
          additional.push({key: this.props.langs[lang[i]], name: lang[i], value: translations[lang[i]]})
        else
          additional.push({key: this.props.langs[lang[i]], name: lang[i], value: ""})
    ...
    this.changeCountry(data.country)
    var tt = [];
    ...
    this.setState({therapies: tt})
  }

	editDeliveryMethod(obj)
  {
    obj = obj.toJS();
    ...
    for(var i=0; i<this.state.additional.length; i++){
      if(!this.state.checked)
        translations[this.state.additional[i].name] = this.state.additional[i].value
      else
        translations[this.state.additional[i].name] = this.state.code

      if(translations[this.state.additional[i].name])
        delete obj[this.state.additional[i].name]
    }
    ...
    if(this.state.image.length != 0)
    {
      deliverymethod_data.append("post_image", this.state.image);
		...
		this.props.EditExistingDeliveryMethod(obj, deliverymethod_data);
    console.log(obj);
	}
  ...
  renderLangs(row, index){
    if(this.state.checked){
      return(
        <div key={index}>
          <div>
            <label className="form-label">{row.key}</label>
            <input name={row.name} type="text" disabled={true} value={''} className={`form-control`}
            onChange={this.handleChange.bind(this, index, row.key, row.name)}/>
          </div>
        </div>
      )
    }
      ...
      return(
        <div key={index}>
          <div>
            <label className="form-label">{row.key}</label>
            <input name={row.name} type="text" disabled={false} value={row.value} className={`form-control`}
            onChange={this.handleChange.bind(this, index, row.key, row.name)}/>
          </div>
        </div>
      )
    ...
    const img_style = {
      ...
      style={otherModalStyles}>
        ...
        <header className={`confirm_box clearfix`}>
          <h2 className="center">Uredi način dostave</h2>
        </header>
      	<form onSubmit={handleSubmit(this.editDeliveryMethod.bind(this))} >
          <div className={`modal-body row`}>
            ...
            <div className="col-lg-6">
              ...
              {this.state.additional.map(this.renderLangs.bind(this))}
            </div>
            <div className="col-lg-6">
