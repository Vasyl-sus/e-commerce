...
class AccessoriesWrap extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      index: 0,
      acc: this.props.accessories && this.props.accessories.filter(row => row.status === 1).map((row, index) => (
        <Link
          key={index}
          href={`/accessory-page`}
  ...
  render() {
    ...
    return (
      ...
      <div className="container page-title-desc">
        ...
        <div className="row list-acc justify-content-center">
          <AliceCarousel
            mouseDragEnabled
            responsive={{
              0: { items: 2 },
            ...
            items={this.state.acc}
            dotsDisabled = {true}
            buttonsDisabled = {true}
            ref={(el) => (this.Carousel = el)}
          />
          <p onClick={() => this.pageHandler(this.Carousel.state.currentIndex - 1)} className="car-button prev pointer"><i></i></p>
          <p onClick={() => this.pageHandler(this.Carousel.state.currentIndex + 1)} className="car-button next pointer"><i></i></p>
        </div>
      </div>
      </React.Fragment>
    );
